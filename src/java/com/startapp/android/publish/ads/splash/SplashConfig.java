...
implements Serializable {
    ...
    @com.startapp.common.c.f(b="Lcom/startapp/android/publish/ads/splash/SplashConfig$MaxAdDisplayTime;")
    ...
    @com.startapp.common.c.f(b="Lcom/startapp/android/publish/ads/splash/SplashConfig$MinSplashTime;")
    private MinSplashTime defaultMinSplashTime = VALUE_DEFAULT_MINSPLASHTIME;
    @com.startapp.common.c.f(b="Lcom/startapp/android/publish/ads/splash/SplashConfig$Orientation;")
    private Orientation defaultOrientation = VALUE_DEFAULT_ORIENTATION;
    @com.startapp.common.c.f(b="Lcom/startapp/android/publish/ads/splash/SplashConfig$Theme;")
     ...
     */
    private void setSplashColorsByTheme(Theme var1_1) {
        var2_2 = 1.$SwitchMap$com$startapp$android$publish$ads$splash$SplashConfig$Theme[var1_1.ordinal()];
        var3_3 = "#333333";
        var4_4 = "#066CAA";
        switch (var2_2) {
            default: {
        ...
        var3_3 = "#FFFFFF";
lbl27: // 6 sources:
        this.splashBgColor = var4_4;
        this.splashFontColor = var3_3;
    }
    ...
    protected View getLayout(Context context) {
        ...
        try {
            ...
            com.startapp.android.publish.adsCommon.f.f.a(context, com.startapp.android.publish.adsCommon.f.d.b, "SplashConfig.getLayout - System service failed", exception.getMessage(), STRING_EMPTY_VALUE);
            return null;
        }
    ...
    protected d initSplashHtml(Activity activity) {
        this.setSplashColorsByTheme(this.getTheme());
        d d2 = new d(activity);
        d2.b(this);
        d2.a(this);
        return d2;
    ...
    protected void initSplashLogo(Activity activity) {
        if (this.getLogo() == null && this.getLogoRes() == -1 && this.getLogoByteArray() != null) {
            ...
            this.setLogo((Drawable)new BitmapDrawable(activity.getResources(), bitmap));
        }
    }
        ...
        if (1.$SwitchMap$com$startapp$android$publish$ads$splash$SplashConfig$Theme[this.getTheme().ordinal()] != 7) {
            ...
            if (this.getLogo() == null && this.getLogoByteArray() == null) {
                if (this.getLogoRes() == -1) {
                    ...
                    this.setLogo(context.getResources().getDrawable(this.getLogoRes()));
                }
            }
        } else if (this.getCustomScreen() == -1) {
            this.setErrorMsg("StartApp: Exception getting custom screen resource id, make sure it is set");
            return false;
        }
        return true;
    }
    ...
    extends Enum<MaxAdDisplayTime> {
        ...
        public static MaxAdDisplayTime getByIndex(long l2) {
            ...
            for (int j = 0; j < arrmaxAdDisplayTime.length; ++j) {
                if (arrmaxAdDisplayTime[j].getIndex() != l2) continue;
                maxAdDisplayTime = arrmaxAdDisplayTime[j];
            }
        ...
        public static MaxAdDisplayTime getByName(String string) {
            ...
            MaxAdDisplayTime[] arrmaxAdDisplayTime = MaxAdDisplayTime.values();
            for (int j = 0; j < arrmaxAdDisplayTime.length; ++j) {
                if (arrmaxAdDisplayTime[j].name().toLowerCase().compareTo(string.toLowerCase()) != 0) continue;
                maxAdDisplayTime = arrmaxAdDisplayTime[j];
            }
            return maxAdDisplayTime;
        }
    ...
    extends Enum<MinSplashTime> {
        ...
        public static MinSplashTime getByIndex(long l2) {
            ...
            for (int j = 0; j < arrminSplashTime.length; ++j) {
                if (arrminSplashTime[j].getIndex() != l2) continue;
                minSplashTime = arrminSplashTime[j];
            }
        ...
        public static MinSplashTime getByName(String string) {
            ...
            MinSplashTime[] arrminSplashTime = MinSplashTime.values();
            for (int j = 0; j < arrminSplashTime.length; ++j) {
                if (arrminSplashTime[j].name().toLowerCase().compareTo(string.toLowerCase()) != 0) continue;
                minSplashTime = arrminSplashTime[j];
            }
            return minSplashTime;
        }
    ...
    extends Enum<Orientation> {
        ...
        public static Orientation getByIndex(int n2) {
            ...
            for (int j = 0; j < arrorientation.length; ++j) {
                if (arrorientation[j].getIndex() != n2) continue;
                orientation = arrorientation[j];
            }
            return orientation;
        }
        ...
        public static Orientation getByName(String string) {
            ...
            Orientation[] arrorientation = Orientation.values();
            for (int j = 0; j < arrorientation.length; ++j) {
                if (arrorientation[j].name().toLowerCase().compareTo(string.toLowerCase()) != 0) continue;
                orientation = arrorientation[j];
            }
            return orientation;
        }
    ...
    extends Enum<Theme> {
        ...
        public static Theme getByIndex(int n2) {
            ...
            for (int j = 0; j < arrtheme.length; ++j) {
                if (arrtheme[j].getIndex() != n2) continue;
                theme = arrtheme[j];
            }
            return theme;
        }
        ...
        public static Theme getByName(String string) {
            ...
            Theme[] arrtheme = Theme.values();
            for (int j = 0; j < arrtheme.length; ++j) {
                if (arrtheme[j].name().toLowerCase().compareTo(string.toLowerCase()) != 0) continue;
                theme = arrtheme[j];
            }
            return theme;
        }
